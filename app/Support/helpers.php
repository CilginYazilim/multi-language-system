...
if (!function_exists('t')) {
    /**
     * Çeviri: <?= t('common.save') ?>
     *
     * Kısa bir ad seçtik çünkü görünümlerde YÜZLERCE kez geçer.
     * "Translator::get(...)" yazmak, HTML'i okunmaz hale getirirdi.
     *
     * @param array<string,string|int|float> $replace Yer tutucular
     */
...
if (!function_exists('te')) {
    /**
     * Çeviri + HTML kaçışlama: <?= te('common.save') ?>
     *
     * NEDEN AYRI BİR FONKSİYON?
     * Çeviri metinleri genellikle geliştirici tarafından yazılır ve
     * güvenlidir; ama çevirmenden gelen bir dosya da olabilir ve
     * içinde yer tutucuyla GELEN kullanıcı verisi bulunabilir:
     *
     *      t('common.welcome', ['name' => $user->name])
     *
     * Burada $user->name güvenilmez veridir. Kullanıcı verisi
     * geçirdiğiniz her çeviride te() kullanın.
     *
     * @param array<string,string|int|float> $replace
     */
    function te(string $key, array $replace = []): string
    {
        return e(App\Core\Translator::get($key, $replace));
    }
}

if (!function_exists('n')) {
    /**
     * Çoğul biçimli çeviri: <?= n('common.records', 5) ?>
     *
     * Sayıyı cümleye elle eklemek ("$count . ' kayıt'") çeviriyi
     * imkânsız hale getirir: her dilde sözcük sırası ve çoğul kuralı
     * farklıdır. Sayıyı yer tutucu olarak vermek şarttır.
     *
     * @param array<string,string|int|float> $replace
     */
    function n(string $key, int $count, array $replace = []): string
    {
        return App\Core\Translator::choice($key, $count, $replace);
    }
}

if (!function_exists('locale')) {
    /** Bu istekte kullanılan dil kodu. */
    function locale(): string
    {
        return App\Core\Translator::locale();
    }
}

if (!function_exists('is_rtl')) {
    /** Metin sağdan sola mı akıyor? (Arapça, İbranice…) */
    function is_rtl(): bool
    {
        return App\Core\Translator::isRtl();
    }
}

if (!function_exists('lang_url')) {
    /**
     * Mevcut sayfanın başka bir dildeki adresi.
     *
     * KULLANICIYI ANA SAYFAYA ATMIYORUZ. Dil değiştiren kişi
     * bulunduğu sayfada kalmak ister; onu kök adrese göndermek,
     * 4. sayfadaki bir listeyi baştan bulmasını gerektirir.
     * Bu yüzden mevcut rotayı ve TÜM sorgu parametrelerini
     * koruyoruz.
     */
    function lang_url(string $locale): string
    {
        ...
        // Rota "r" parametresinde taşınır; url() onu kendisi ekler.
        unset($params['r']);

        $params['lang'] = $locale;

        return url(App\Core\Router::currentPath(), $params);
    }
}

if (!function_exists('local_number')) {
    /** Sayıyı yerel biçimde yazar: 1.234,56 (tr) · 1,234.56 (en) */
    function local_number(float $value, int $decimals = 0): string
    {
        return App\Core\Translator::number($value, $decimals);
    }
}

if (!function_exists('local_date')) {
    /** Tarihi yerel biçimde yazar: 30.08.2026 (tr) · Aug 30, 2026 (en) */
    function local_date(?string $value, bool $withTime = false): string
    {
        if ($value === null || $value === '') {
            return '—';
        }

        return App\Core\Translator::date($value, $withTime);
    }
}

if (!function_exists('local_money')) {
    /**
     * Parayı yerel biçimde yazar.
     *
     * DİKKAT: Yalnızca BİÇİMLENDİRME yapar; kur dönüşümü YAPMAZ.
     * 100 TL'yi "$100" gibi göstermek ciddi bir hatadır.
     */
    function local_money(float $value, string $currency = 'TRY'): string
    {
        return App\Core\Translator::money($value, $currency);
    }
}
