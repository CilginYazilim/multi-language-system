-- =====================================================================
--  Çoklu Dil (i18n) Sistemi – VERİTABANI KURULUM DOSYASI
--  cilginyazilim.com
-- ---------------------------------------------------------------------
--  KURULUM
...
CREATE DATABASE IF NOT EXISTS `cy_i18n`
...
-- ===============================================================
--  PROJEYE ÖZEL ŞEMA DEĞİŞİKLİĞİ
-- ---------------------------------------------------------------
--  Bu proje yeni bir TABLO eklemez; var olan "users" tablosuna
--  bir SÜTUN ekler.
--
--  DİL TERCİHİ NEDEN HESAPTA?
--  Yalnızca çerezde tutulsaydı, kullanıcı işten eve geçtiğinde,
--  başka bir tarayıcı açtığında veya çerezleri temizlediğinde
--  tercihi kaybolurdu. Hesaba bağlayınca nereden girerse girsin
--  kendi dilini bulur.
--
--  Çerez yine de yazılır ama artık farklı bir işi vardır: sunucu,
--  sayfanın İLK HTML'ini doğru dil ve yönle üretebilsin diye
--  (giriş yapmamış ziyaretçi için de tek kaynak odur).
-- ===============================================================

-- ENUM kullanmak, veritabanı seviyesinde de BEYAZ LİSTE sağlar:
-- desteklenmeyen bir dil kodu hiç kaydedilemez. Uygulama da
-- kontrol eder (User::normalizeLocale) ama son söz burasıdır.
--
-- YENİ DİL EKLERKEN ÜÇ YERE DOKUNULUR:
--   1. Translator::LOCALES  (kod + ad + yön)
--   2. lang/<kod>/common.php (sözlük)
--   3. Bu ENUM              (ALTER TABLE ile)
-- Üçüncüsünü unutmak, tercihin sessizce kaydedilmemesine yol açar.
ALTER TABLE `users`
    ADD COLUMN `locale` ENUM('tr','en','ar') NOT NULL DEFAULT 'tr'
    AFTER `theme`;

-- Demo kullanıcılarının bir kısmını farklı dillere alıyoruz;
-- "hesap tercihi çerezi ezer mi?" sorusunu deneyebilirsiniz.
UPDATE `users` SET `locale` = 'en' WHERE `id` IN (2, 4, 6, 8, 10);
UPDATE `users` SET `locale` = 'ar' WHERE `id` IN (3, 5, 7);
