 ...
 *  PARÇA: Sol menü
 * ---------------------------------------------------------------------
 *  Menü öğeleri config/menu.php dosyasından okunur ve etiketleri
 *  ÇEVİRİ ANAHTARIDIR ("common.nav.users"), düz metin değil.
 *  Böylece menü de dil değiştiğinde birlikte değişir — yarısı
 *  çevrilmiş bir arayüz, hiç çevrilmemiş olandan kötü görünür. Yeni bir sayfa
    ...
    <nav class="cy-sidebar__nav">
        <?php foreach ($menu as $group): ?>
            <div class="cy-nav-group">
                <span class="cy-nav-group__label"><?= te($group['label']) ?></span>

                <?php foreach ($group['items'] as $item): ?>
                    <a class="cy-nav-link<?= is_route($item['route']) ? ' is-active' : '' ?>"
                       href="<?= e(url($item['route'])) ?>"
                       data-title="<?= te($item['label']) ?>"
                       <?= is_route($item['route']) ? 'aria-current="page"' : '' ?>>
                        <?= icon($item['icon']) ?>
                        <span class="cy-nav-link__text"><?= te($item['label']) ?></span>
                    </a>
                <?php endforeach; ?>
            </div>
        <?php endforeach; ?>
