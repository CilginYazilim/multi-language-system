...
?>
<nav class="cy-pager" aria-label="<?= te('common.pagination.page', ['page' => $paginator->currentPage()]) ?>">
    ...
    <p class="cy-pager__info">
        <?php if ($paginator->total() === 0): ?>
            <?= te('common.pagination.empty') ?>
        <?php else: ?>
            <?php /* Sayılar YEREL BİÇİMDE yazılır: 1.234 (tr) · 1,234 (en).
                     Cümlenin tamamı da çeviriden gelir; parçalayıp
                     birleştirseydik sözcük sırası farklı olan dillerde
                     düzgün cümle kurulamazdı. */ ?>
            <?= t('common.pagination.summary', [
                'from'  => local_number((float) $paginator->from()),
                'to'    => local_number((float) $paginator->to()),
                'total' => local_number((float) $paginator->total()),
            ]) ?>
        <?php endif; ?>
    </p>
        ...
        <ul class="cy-pager__list">
            ...
            <li>
                ...
                <?php else: ?>
                    <a class="cy-pager__link"
                       ...
                       rel="prev" aria-label="<?= te('common.pagination.previous') ?>">
                        <?= icon('chevron', 'cy-icon cy-icon--sm cy-pager__prev') ?>
                    </a>
                <?php endif; ?>
            </li>
                ...
                <li<?= $page === null ? '' : ' class="cy-pager__num"' ?>>
                    ...
                    <?php else: ?>
                        <a class="cy-pager__link"
                           ...
                           aria-label="<?= te('common.pagination.page', ['page' => $page]) ?>"><?= $page ?></a>
                    <?php endif; ?>
                </li>
            <?php endforeach; ?>
            ...
            <li>
                ...
                <?php else: ?>
                    <a class="cy-pager__link"
                       href="<?= e($paginator->url($paginator->currentPage() + 1, $route, $query)) ?>"
                       rel="next" aria-label="<?= te('common.pagination.next') ?>">
                        <?= icon('chevron', 'cy-icon cy-icon--sm') ?>
                    </a>
                <?php endif; ?>
            </li>
