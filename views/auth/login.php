...
<div class="cy-auth">
    ...
    <aside class="cy-auth__aside">
        ...
        <div class="cy-auth__pitch">
            <h2>Tek kod tabanı, birden çok dil</h2>
            <p>Dosya tabanlı çeviri sözlüğü, çoğul kuralları, sayı ve tarih yerelleştirmesi, RTL desteği ve dil seçimini hesaba kaydeden örnek PHP çoklu dil altyapısı.</p>

            <div class="cy-auth__features">
                <span class="cy-auth__feature"><?= icon('activity', 'cy-icon cy-icon--sm') ?> Dosya tabanlı sözlük, yedek dile düşme</span>
                <span class="cy-auth__feature"><?= icon('users', 'cy-icon cy-icon--sm') ?> Çoğul kuralları ve yer tutucular</span>
                <span class="cy-auth__feature"><?= icon('calendar', 'cy-icon cy-icon--sm') ?> Sayı, tarih ve para birimi yerelleştirmesi</span>
                <span class="cy-auth__feature"><?= icon('shield', 'cy-icon cy-icon--sm') ?> RTL (sağdan sola) yerleşim desteği</span>
            </div>
        </div>
    ...
    <main class="cy-auth__panel">
        <div class="cy-auth__form">
            ...
            <?php /* DİL SEÇİCİ GİRİŞ EKRANINDA DA OLMALIDIR.
                     Arayüzü okuyamayan biri giriş yapamaz; dili
                     ancak giriş yaptıktan sonra değiştirebilmek
                     tavuk-yumurta sorunudur. */ ?>
            <div class="cy-auth__lang">
                <?php App\Core\View::partial('partials/langswitch'); ?>
            </div>

            <div class="cy-auth__logo-mobile">
                <img src="<?= e(asset('images/logo.png')) ?>" alt="">
                <strong><?= e($appName ?? 'CY i18n') ?></strong>
            </div>

            <h1 class="cy-title mb-1"><?= te('common.auth.title') ?></h1>
            <p class="cy-subtitle mb-4"><?= te('common.auth.subtitle') ?></p>

            <form method="post" action="<?= e(url('login')) ?>" novalidate id="cy_login_form">
                <?= csrf_field() ?>

                <div class="mb-3">
                    <label class="form-label" for="email"><?= te('common.auth.email') ?></label>
                ...
                <div class="mb-3">
                    <label class="form-label" for="password"><?= te('common.auth.password') ?></label>
                ...
                <div class="cy-remember">
                    <div class="form-check">
                        ...
                        <label class="form-check-label" for="remember">
                            <?= te('common.auth.remember') ?>
                        </label>
                    </div>

                    <span class="cy-remember__hint"><?= te('common.auth.remember_hint') ?></span>
                </div>

                <button type="submit" class="btn cy-btn cy-btn--primary cy-btn--block">
                    <?= icon('logout', 'cy-icon cy-icon--sm') ?> <?= te('common.auth.submit') ?>
                </button>
            </form>

            <!-- ---- Demo hesaplar (gerçek projede kaldırın) ---- -->
            <div class="cy-demo">
                <span class="cy-eyebrow"><?= te('common.auth.demo_accounts') ?></span>
